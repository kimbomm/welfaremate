...
- 타깃 재가공(targets): 스냅샷 조건 텍스트에서 나이·기초수급 등 플래그 추출 → welfare-targets.json. 검색 필터(나이대·소득)에서 스냅샷 + targets 병합 사용.
...
| 필터 | 우측 슬라이드 드로어. 지역(내 지역/전국), 카테고리·혜택 유형·신청 기한 칩(복수 선택), 가구수·소득·나이대·대상 특성 셀렉트박스(선택안함 옵션 포함). 기본값은 온보딩(프로필) 반영. 필터는 sessionStorage 유지(상세 진입 후 뒤로가기 시 유지). |
| 소득 필터 | 중위소득 50/100/150% **이상** 기준. 기초수급·차상위 전용 혜택은 소득 필터 시 제외. |
| 나이대 필터 | 선택안함 / 내 나이에 맞는 혜택만. 스냅샷 eligibility.age + target 재가공(welfare-targets.json) age 병합 사용. |
| 대상 특성 | 선택안함 + 청년·한부모·장애인 등. "전체" 제거. |
| 정렬 | 추천 / 마감 / 일반. 순서: 추천(미마감) > 없음(미마감) > 마감+추천 > 마감 |
| 결과 표시 | 무한 스크롤 (20개씩, throttle 400ms), 카드에 추천/마감 뱃지. 페이지 이동·필터 적용 시 스크롤 최상단. |
